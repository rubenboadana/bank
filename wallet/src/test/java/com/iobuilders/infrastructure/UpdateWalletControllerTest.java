...
class UpdateWalletControllerTest {
    ...
    void should_responseWalletNotFound_when_invalidWalletIdProvided() throws Exception {
        //Given
        doThrow(new CommandExecutionException("Command handler thrown an exception", new RuntimeException())).when(commandBusMock).send(any(DepositMoneyInWalletCommand.class));
    ...
    void should_returnHTTP200_when_depositIntoWalletSucceed() throws Exception {
        ...
        doReturn(completedFuture).when(commandBusMock).send(any(DepositMoneyInWalletCommand.class));

        SecurityContextHolder.setContext(SecurityContextHolder.createEmptyContext());
        SecurityContextHolder.getContext().setAuthentication(new TestingAuthenticationToken(USERNAME, null));

        //When/Then
        MvcResult mvcResult = this.mockMvc.perform(put("/wallets/" + WALLET_ID).contentType(MediaType.APPLICATION_JSON).content(objectMapper.writeValueAsString(new Quantity(NEW_WALLET_QUANTITY))))
